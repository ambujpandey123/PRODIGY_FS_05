...
const SignupPage = () => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const SignupResponse = await LoginUser(name, email, password, rememberMe) as SignupResponse;
        if (SignupResponse.success === false || SignupResponse.success === undefined) {
            setDataError([true, SignupResponse.message ?? "Something went wrong, please try again later"] as DataErrorState);
            toast.error(SignupResponse.message ?? "Something went wrong, please try again later");
            console.log(SignupResponse.message ?? "Something went wrong, please try again later");
            setLoader(false);
            return;
        }
        setLoader(false);
        setDataError([false, SignupResponse?.message ?? ""] as DataErrorState);
        toast.success(SignupResponse.message ?? "Login successful");
        setEmail('');
        setPassword('');
        setName('');
        setconfirmpassword('');
